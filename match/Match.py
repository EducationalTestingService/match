    ...
    **First**, we simply reverse tokenization on the string ``" ".join(["Dogs", "make", "great", "pets", "."])``
    ...
    if type(word_or_token_list_to_match) is list:
        ...
        if len(matches) == 0:
            matches = [
                ...
                for m in re.finditer(
                    "\s*".join(re.escape(w) for w in word_or_token_list_to_match),
                    original_text,
                    regex_flags,
                )
            ]
            if len(matches) == 0:
                ...
                matches = [
                    (m.start(), m.end(), original_text[m.start() : m.end()])
                    for m in re.finditer(
                        re.escape(edit_distance_match), clean_text, regex_flags
                    )
                ]
                if len(matches) == 0:
                    edit_distance_match = _match_by_edit_distance(
                    ...
                    matches = [
                        (m.start(), m.end(), original_text[m.start() : m.end()])
                        for m in re.finditer(
                            re.escape(edit_distance_match), original_text, regex_flags
                        )
                    ]
                    if len(matches) == 0:
                        edit_distance_match = _match_by_edit_distance(
        ...
        matches = [
            (m.start(), m.end(), original_text[m.start() : m.end()])
            for m in re.finditer(
                r"\b" + re.escape(word_or_token_list_to_match) + r"\b",
                clean_text,
                regex_flags,
            )
        ]
...
def untokenize(text):
    ...
    # '\s' escape is not allowed in the second argument
    # use '\\s' instead
    ...
    for (unicode_char, ascii_char) in non_ascii:
        try:
            cleaned = cleaned.replace(
                bytes(unicode_char, "utf-8"), bytes(ascii_char, "utf-8")
            )
        except:
            cleaned = cleaned.replace(
                unicode_char.encode("utf-8"), ascii_char.encode("utf-8")
            )
...
def _match_by_edit_distance(full_text, text_to_match):
    ...
    try:
        end_point = (
            text_to_match.index(" ") if " " in text_to_match else len(text_to_match)
        )
        potential_matches = [
            full_text[m.start() : (m.start() + len(text_to_match) + 1)]
            for m in re.finditer(
                re.escape(text_to_match[0:end_point]), full_text, re.U | re.I
            )
        ]
    except:
        import sys
    ...
    if len(potential_matches) == 0:
        potential_matches = [
            full_text[m.start() : (m.start() + len(text_to_match) + 1)]
            for m in re.finditer(re.escape(text_to_match[0]), full_text, re.U)
        ]
    if len(potential_matches) == 0:
    ...
    potential_matches = [
        (
            p[0 : p.rindex(text_to_match[-1]) + 1]
